...
V2.10 수정내용
1. Minute % 30 < 5로 30분 주기 판정 버그 수정
2. 사이트 실패/미갱신 시 "5분대기중" 명시적으로 표시 (기존엔 조용히 아무것도 안 함)
3. 사이트가 아직 안 바뀐 값(현재와 동일) 줄 때 "5분대기중"으로 대체 표시
4. 0~5분/5분 이후 양방향 상호 폴백 추가

V2.09 수정내용
1. 테러존 5-3 얼어붙은 동토 추가

V2.08 수정내용
1. 테러존 메뉴 메인창으로 이동
2. 테러존 10분 Delay제거(아직 불안정 할수 있습니다.)

V2.07 수정내용
1. 카카오 회원가입 오류 수정

V2.06 수정내용
1. 카카오 인증에 유튜브 태그 입력 기능 추가
2. 멀티로더 최상단 윈도우 위치 기능 해제
...
오토마우스 기능은 매니저, 멤버쉽, 운영자 대상의 기능이며 카카오 로그인후 사용 가능합니다.
(기능 활성화가 되지 않으면 부드로더 오픈채팅방에 들어 오셔서 권한 요청 부탁드립니다.) 
